...
// common code in there (like making an LinkedList with a few items in it) without having to put
...
TEST_F(LinkedListTestFixture, Remove)
{
	...
	int num = slist2.remove_all("X");
	EXPECT_EQ(num, num_added);
}

TEST_F(LinkedListTestFixture, AtVsBrackets)
{
...
TEST_F(LinkedListTestFixture, Assignment)
{
	...
	// ... now do the thing
	slist2 = slist2;		// Could cause problems if not accounted for!		//problem here
...
TEST_F(LinkedListTestFixture, CustomStrings)
{
